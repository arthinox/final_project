...
void setLED(int LED)
{
	IOWR_ALTERA_AVALON_PIO_DATA(0x3180, (IORD_ALTERA_AVALON_PIO_DATA(0x3180) | (0x001 << LED)));
}
...
void printSignedHex0(signed char value)
{
	BYTE tens = 0;
	BYTE ones = 0;
	WORD pio_val = IORD_ALTERA_AVALON_PIO_DATA(0x3190);
	if (value < 0)
	{
		setLED(11);
		value = -value;
	...
	IOWR_ALTERA_AVALON_PIO_DATA(0x3190, pio_val);
}
...
void printSignedHex1(signed char value)
{
	BYTE tens = 0;
	BYTE ones = 0;
	DWORD pio_val = IORD_ALTERA_AVALON_PIO_DATA(0x3190);
	if (value < 0)
	{
		setLED(10);
		value = -value;
